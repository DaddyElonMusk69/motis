...
Auto-generated from vibe trading preset: social_alpha_team.yaml
...
class State(TypedDict):
    target: str  # Focus name or market (e.g. BTC, Tesla, A-share tech, Nasdaq)
    timeframe: str  # Horizon (real-time / daily / weekly)
    task_twitter_summary: str  # Output from twitter_analyst
    task_telegram_summary: str  # Output from telegram_analyst
    task_reddit_summary: str  # Output from reddit_analyst
    task_alpha_synthesis_summary: str  # Output from alpha_synthesizer
...
MANIFEST = {
    "name": "Social-Media Alternative Data Team",
    "version": 1,
    "type": "research",
    ...
    "agents": {
        "twitter_analyst": {
            ...
            "system_prompt": """You are an alternative-data analyst focused on the FinTwit ecosystem—KOL views, topic heat, and sentiment extremes—extracting predictive signal from social noise.
...
Use read_url for X/FinTwit sources.""",
        ...
        "telegram_analyst": {
            ...
            "system_prompt": """You are an alternative-data analyst for Telegram crypto/quant communities—signal quality, message heat, and alpha leads from high-volume channels.
...
Use read_url for Telegram and Nansen/Dune.""",
        ...
        "reddit_analyst": {
            "role": "Reddit Analyst",
            "system_prompt": """You are an alternative-data analyst for Reddit finance—WSB / investing / crypto—hot names, unusual options, and retail sentiment quant; systematic contrarian and small-cap attention signals.
...
Use read_url for Reddit pages/API data.""",
        ...
        "alpha_synthesizer": {
            ...
            "system_prompt": """You are a senior quant alt-data researcher—turning multi-channel social data into quantifiable alpha: factor design, synthesis, validity tests, and trade rules.
...
Use factor_analysis for factor stats.""",
    ...
    "nodes": [
        {"name": "task_twitter", "type": "REASON", "agent": "twitter_analyst"},
        {"name": "task_telegram", "type": "REASON", "agent": "telegram_analyst"},
        {"name": "task_reddit", "type": "REASON", "agent": "reddit_analyst"},
        {"name": "task_alpha_synthesis", "type": "REASON", "agent": "alpha_synthesizer"},
    ],
...
async def task_twitter(state: State) -> dict:
    """REASON: Twitter Analyst"""
    ...
    result = await run_agent("twitter_analyst", context)
    log_event("task_twitter", f"Completed: {len(result.summary)} chars")
    return {"task_twitter_summary": result.summary}


async def task_telegram(state: State) -> dict:
    """REASON: Telegram Analyst"""
    # Build context from state
    context = {
        "target": state.get("target", ""),
        "timeframe": state.get("timeframe", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("telegram_analyst", context)
    log_event("task_telegram", f"Completed: {len(result.summary)} chars")
    return {"task_telegram_summary": result.summary}


async def task_reddit(state: State) -> dict:
    """REASON: Reddit Analyst"""
    # Build context from state
    context = {
        "target": state.get("target", ""),
        "timeframe": state.get("timeframe", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("reddit_analyst", context)
    log_event("task_reddit", f"Completed: {len(result.summary)} chars")
    return {"task_reddit_summary": result.summary}


async def task_alpha_synthesis(state: State) -> dict:
    ...
    context = {
        ...
        "timeframe": state.get("timeframe", ""),
        # Upstream summaries
        "twitter_report": state.get("task_twitter_summary", ""),
        "telegram_report": state.get("task_telegram_summary", ""),
        "reddit_report": state.get("task_reddit_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_twitter_summary"):
        upstream_parts.append("## Twitter Report\n" + state["task_twitter_summary"])
    if state.get("task_telegram_summary"):
        upstream_parts.append("## Telegram Report\n" + state["task_telegram_summary"])
    if state.get("task_reddit_summary"):
        upstream_parts.append("## Reddit Report\n" + state["task_reddit_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("alpha_synthesizer", context)
    log_event("task_alpha_synthesis", f"Completed: {len(result.summary)} chars")
    return {"task_alpha_synthesis_summary": result.summary}
...
def build_graph():
    g = StateGraph(State)
    g.add_node("task_twitter", task_twitter)
    g.add_node("task_telegram", task_telegram)
    g.add_node("task_reddit", task_reddit)
    g.add_node("task_alpha_synthesis", task_alpha_synthesis)

    g.set_entry_point("task_reddit")
    g.add_edge("task_twitter", "task_alpha_synthesis")
    g.add_edge("task_telegram", "task_alpha_synthesis")
    g.add_edge("task_reddit", "task_alpha_synthesis")

    # Parallel entry: fan-out from first node to siblings
    g.add_edge("task_reddit", "task_telegram")
    g.add_edge("task_reddit", "task_twitter")
    g.add_edge("task_alpha_synthesis", END)
