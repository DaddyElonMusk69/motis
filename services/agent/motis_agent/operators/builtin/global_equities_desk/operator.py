...
Auto-generated from vibe trading preset: global_equities_desk.yaml
...
class State(TypedDict):
    goal: str  # Investment objective (e.g., Q2 2026 global equity allocation, tech sector deep-dive)
    risk_tolerance: str  # Risk tolerance level (conservative / moderate / aggressive)
    task_ashare_summary: str  # Output from a_share_researcher
    task_ushk_summary: str  # Output from us_hk_researcher
    task_crypto_summary: str  # Output from crypto_researcher
    task_strategy_summary: str  # Output from global_strategist
...
MANIFEST = {
    "name": "Global Equities Research Desk",
    "version": 1,
    "type": "research",
    ...
    "agents": {
        "a_share_researcher": {
            ...
            "system_prompt": """You are a senior A-share equity researcher at a top-tier fund, with deep expertise in China domestic markets — sector rotation, policy-driven catalysts, Northbound flow patterns, and A-share-specific behavioral dynamics.
...
Use factor_analysis tool for quantitative factor scoring.""",
        ...
        "us_hk_researcher": {
            ...
            "system_prompt": """You are a senior US and HK equity researcher at a global fund, with expertise in SEC filings, earnings revision analysis, ETF flow tracking, and cross-listing dynamics. You cover both Hong Kong and US equities with an institutional lens.
...
Use load_skill for yfinance data, SEC filing analysis, earnings revision patterns, ETF flow analysis, and cross-listing dynamics.""",
        ...
        "crypto_researcher": {
            ...
            "system_prompt": """You are a senior crypto researcher covering major digital assets from a cross-market allocation perspective. You focus on identifying crypto opportunities that complement traditional equity positioning.
...
Use load_skill for OKX market data, funding rate analysis, and stablecoin flow indicators.""",
        ...
        "global_strategist": {
            ...
            "system_prompt": """You are the chief global equity strategist, responsible for synthesizing A-share, HK/US, and crypto research into a unified multi-market investment recommendation. You make the final call on cross-market allocation, security selection, and risk management.
...
You may use backtest to validate historical performance of the proposed allocation.""",
    ...
    "nodes": [
        {"name": "task_ashare", "type": "REASON", "agent": "a_share_researcher"},
        {"name": "task_ushk", "type": "REASON", "agent": "us_hk_researcher"},
        {"name": "task_crypto", "type": "REASON", "agent": "crypto_researcher"},
        {"name": "task_strategy", "type": "REASON", "agent": "global_strategist"},
    ],
...
async def task_ashare(state: State) -> dict:
    """REASON: A-Share Equity Researcher"""
    ...
    result = await run_agent("a_share_researcher", context)
...
async def task_ushk(state: State) -> dict:
    """REASON: US & HK Equity Researcher"""
    # Build context from state
    context = {
        "goal": state.get("goal", ""),
        "risk_tolerance": state.get("risk_tolerance", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("us_hk_researcher", context)
...
async def task_crypto(state: State) -> dict:
    """REASON: Crypto Asset Researcher"""
    # Build context from state
    context = {
        "goal": state.get("goal", ""),
        "risk_tolerance": state.get("risk_tolerance", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("crypto_researcher", context)
...
async def task_strategy(state: State) -> dict:
    """REASON: Global Equity Strategist"""
    # Build context from state
    context = {
        "goal": state.get("goal", ""),
        "risk_tolerance": state.get("risk_tolerance", ""),
        # Upstream summaries
        "a_share_research": state.get("task_ashare_summary", ""),
        "us_hk_research": state.get("task_ushk_summary", ""),
        "crypto_research": state.get("task_crypto_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_ashare_summary"):
        upstream_parts.append("## A Share Research\n" + state["task_ashare_summary"])
    if state.get("task_ushk_summary"):
        upstream_parts.append("## Us Hk Research\n" + state["task_ushk_summary"])
    if state.get("task_crypto_summary"):
        upstream_parts.append("## Crypto Research\n" + state["task_crypto_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("global_strategist", context)
...
def build_graph():
    ...
    g.add_edge("task_ashare", "task_strategy")
    g.add_edge("task_ushk", "task_strategy")
    g.add_edge("task_crypto", "task_strategy")
