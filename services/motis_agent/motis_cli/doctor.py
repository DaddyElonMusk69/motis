"""
Doctor command for motis CLI.

Diagnoses issues with Motis setup.
"""
...
from motis_constants import OPENROUTER_MODELS_URL
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Motis Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'motis setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'motis setup' to configure API keys")
                fixed_count += 1
            else:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'motis setup' to migrate config")
                else:
                    issues.append("Run 'motis doctor --fix' or 'motis setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'motis doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from motis_cli.auth import get_codex_auth_status
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Motis a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Motis Persona\n\n"
                "<!-- Edit this file to customize how Motis communicates. -->\n\n"
                "You are Motis, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'motis doctor --fix' to checkpoint")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint; skip the direct probe.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'motis setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: motis skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: motis memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: motis memory setup")
                issues.append("No Honcho API key — run 'motis memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run motis memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: motis memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: motis memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from motis_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "motis -p" in content:
                            _m = _re.search(r"motis -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'motis doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
