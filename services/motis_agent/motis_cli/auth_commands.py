...
import motis_cli.auth as auth_mod
...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        raise SystemExit("Provider 'nous' is no longer supported by Motis.")
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "openai-codex", "qwen-oauth"} else AUTH_TYPE_API_KEY
    ...
    raise SystemExit(f"`motis auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from motis_cli.config import remove_env_value
    ...
    # re-seeded on the next load_pool() call.
    elif removed.source == "device_code" and provider in ("openai-codex",):
        ...
        oauth_file = get_motis_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Motis Anthropic OAuth credentials")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `motis auth` is called bare."""
...
def _pick_provider(prompt: str = "Provider") -> str:
    ...
    known = sorted(set([*PROVIDER_REGISTRY.keys(), "openrouter"]))
