...
- ``motis_cli.providers``        -- canonical provider identity + overlays
- ``motis_cli.model_normalize``  -- per-provider name formatting
...
from motis_cli.providers import (
    determine_api_mode,
    get_label,
    is_aggregator,
    resolve_provider_full,
)
from motis_cli.model_normalize import (
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Motis. They lack the tool-calling capabilities "
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'motis model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'motis doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from motis_cli/models.py (OPENROUTER_MODELS,
    ...
    from motis_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by motis provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # --- 1. Check Motis-mapped providers ---
    ...
    # --- 2. Check Motis-only providers (openai-codex, copilot) ---
    from motis_cli.providers import HERMES_OVERLAYS
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if overlay.auth_type in ("oauth_device_code", "oauth_external"):
            # These use auth stores, not env vars — check for auth.json entries
            try:
                from motis_cli.auth import _load_auth_store
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: motis setup -> Custom OpenAI-compatible endpoint"
            ),
        )
