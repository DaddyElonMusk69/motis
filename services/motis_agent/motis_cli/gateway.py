"""
Gateway subcommand for motis CLI.

Handles: motis gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from motis_cli.config import get_env_value, get_motis_home, save_env_value, is_managed, managed_error
# display_motis_home is imported lazily at call sites to avoid ImportError
# when motis_constants is cached from a pre-update version during `motis update`.
from motis_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "motis-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None) -> list:
    ...
    patterns = [
        "motis_cli.main gateway",
        "motis_cli/main.py gateway",
        "motis gateway",
        "gateway/run.py",
    ]
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    motis gateway uninstall")
    print_info("    sudo motis gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Motis can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo motis gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo motis gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo motis gateway start --system")
            return scope, False
...
def get_motis_cli_path() -> str:
    """Get the path to the motis CLI."""
    # Check if installed via pip
    import shutil
    motis_bin = shutil.which("motis")
    if motis_bin:
        return motis_bin
    
    # Fallback to direct module execution
    return f"{get_python_path()} -m motis_cli.main"
...
User={username}
Group={group_name}
ExecStart={python_path} -m motis_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Motis install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}motis gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}motis gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}motis gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}motis gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}motis gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.motis.gateway-{suffix}" if suffix else "ai.motis.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Motis install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  motis gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Motis install")
    else:
        print("⚠ Service definition is stale relative to the current Motis install")
        print("  Run: motis gateway start")
    ...
    if deep:
        log_file = get_motis_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Motis will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @motis:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. motis) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Motis delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Motis agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Motis will use (e.g., motis@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Motis connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: motis pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_motis_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'motis pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: motis pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'motis gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from motis_cli.main import cmd_whatsapp
    import argparse
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: motis gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                print_info("  Run in foreground: motis gateway")
    else:
        print()
        print_info("No platforms configured. Run 'motis gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("Run manually: motis gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: motis gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  motis gateway install")
                print("  sudo motis gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  motis gateway          # Run in foreground")
                print("  motis gateway install  # Install as user service")
                print("  sudo motis gateway install --system  # Install as boot-time system service")
