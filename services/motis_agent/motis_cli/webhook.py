"""motis webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    motis webhook subscribe <name> [options]
    motis webhook list
    motis webhook remove <name>
    motis webhook test <name> [--payload '{"key": "value"}']
...
def _setup_hint() -> str:
  ...
  Then start the gateway: motis gateway run
"""
...
def webhook_command(args):
    """Entry point for 'motis webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: motis webhook {subscribe|list|remove|test}")
        print("Run 'motis webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (motis gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: motis webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from motis webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (motis gateway run)")
