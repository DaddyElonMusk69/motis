"""
Single source of truth for provider identity in Motis.
...
2. **Motis overlays** — transport type, auth patterns, aggregator flags,
...
# -- Motis overlay ----------------------------------------------------------
# Motis-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Motis-specific provider metadata layered on top of models.dev."""
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "motis", "user-config"
...
def get_overlay(provider_id: str) -> Optional[HermesOverlay]:
    """Get Motis overlay for a provider, if one exists."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Motis overlays (for providers not in models.dev: openai-codex, etc.)
      2. models.dev catalog + Motis overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + motis extra
    ...
    if overlay is not None:
        # Motis-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
LABELS: Dict[str, str] = {
    ...
    # Legacy Motis IDs (point to same providers)
