"""
MCP Server Management CLI — ``motis mcp`` subcommand.

Implements ``motis mcp add/remove/list/test/configure`` for interactive
...
from motis_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_motis_home,  # noqa: F401 — used by test mocks
)
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  motis mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  motis mcp add github --command npx --args @modelcontextprotocol/server-github')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_motis_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: motis mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_motis_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  motis mcp add <name> --url <endpoint>')
        _info('  motis mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'motis mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``motis mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("motis mcp add <name> --url <endpoint>        Add an MCP server")
        _info("motis mcp add <name> --command <cmd>         Add a stdio server")
        _info("motis mcp remove <name>                      Remove a server")
        _info("motis mcp list                               List servers")
        _info("motis mcp test <name>                        Test connection")
        _info("motis mcp configure <name>                   Toggle tools")
        print()
