#!/usr/bin/env python3
"""
Motis CLI - Main entry point.

Usage:
    motis                     # Interactive chat (default)
    motis chat                # Interactive chat
    motis gateway             # Run gateway in foreground
    motis gateway start       # Start gateway as service
    motis gateway stop        # Stop gateway service
    motis gateway status      # Show gateway status
    motis gateway install     # Install gateway service
    motis gateway uninstall   # Uninstall gateway service
    motis setup               # Interactive setup wizard
    motis logout              # Clear stored authentication
    motis status              # Show status of all components
    motis cron                # Manage cron jobs
    motis cron list           # List cron jobs
    motis cron status         # Check if cron scheduler is running
    motis doctor              # Check configuration and dependencies
    motis honcho setup                    # Configure Honcho AI memory integration
    motis honcho status                   # Show Honcho config and connection status
    motis honcho sessions                 # List directory → session name mappings
    motis honcho map <name>               # Map current directory to a session name
    motis honcho peer                     # Show peer names and dialectic settings
    motis honcho peer --user NAME         # Set user peer name
    motis honcho peer --ai NAME           # Set AI peer name
    motis honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    motis honcho mode                     # Show current memory mode
    motis honcho mode [hybrid|honcho|local]  # Set memory mode
    motis honcho tokens                   # Show token budget settings
    motis honcho tokens --context N       # Set session.context() token cap
    motis honcho tokens --dialectic N     # Set dialectic result char cap
    motis honcho identity                 # Show AI peer identity representation
    motis honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    motis honcho migrate                  # Step-by-step migration guide: OpenClaw native → Motis + Honcho
    motis version             Show version
    motis update              Update to latest version
    motis uninstall           Uninstall Motis
    motis sessions browse     Interactive session picker with search

    motis claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (motis tools, motis setup, motis model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'motis {command_name}' requires an interactive terminal.\n"
...
load_motis_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `motis` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from motis_logging import setup_logging as _setup_logging
...
def _has_any_provider_configured() -> bool:
    ...
    from motis_cli.config import get_env_path, get_motis_home, load_config
    from motis_cli.auth import get_auth_status

    # Determine whether Motis itself has been explicitly configured (model
    ...
    from motis_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for any active auth-store provider credentials
    auth_file = get_motis_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Motis has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Motis to use their tokens.
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'motis sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Motis isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  motis setup")
        print()

        from motis_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'motis setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from motis_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from motis_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from motis_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Motis?")
        print()
    ...
    session_dir = get_motis_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: motis gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  motis gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Motis'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  motis gateway")
        ...
        print()
        print("  Or install as a service: motis gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'motis whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from motis_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``motis model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from motis_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from motis_cli.config import load_config, get_env_value
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from motis_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from motis_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from motis_cli.config import get_env_value, save_env_value
    ...
    from motis_cli.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from motis_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from motis_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `motis auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from motis_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from motis_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from motis_cli.auth import _save_model_choice, deactivate_provider
    from motis_cli.config import get_env_value, load_config, save_config
    ...
    from motis_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `motis model` to set a model.")
...
from motis_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from motis_cli.config import save_env_value, load_config, save_config
    from motis_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from motis_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from motis_cli.copilot_auth import validate_copilot_token
...
def _model_flow_kimi(config, current_model=""):
    ...
    from motis_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from motis_cli.config import get_env_value, save_env_value, load_config, save_config
    from motis_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from motis_constants import display_motis_home as _dhh_fn
            print(f"    Motis will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'motis model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Motis CLI with a provider."""
    from motis_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from motis_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from motis_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from motis_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from motis_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from motis_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from motis_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from motis_cli.dump import run_dump
    run_dump(args)


def cmd_config(args):
    """Configuration management."""
    from motis_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Motis v{__version__} ({__release_date__})")
    ...
    try:
        from motis_cli.banner import check_for_updates
        from motis_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from motis_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``motis update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Motis by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/{OFFICIAL_REPO_SLUG}/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="motis-update-")
        zip_path = os.path.join(tmp_dir, f"motis-{branch}.zip")
        ...
        # GitHub ZIPs extract to motis-<branch>/ at the repo root.
        extracted_root = os.path.join(tmp_dir, f"motis-{branch}")
        extracted = os.path.join(extracted_root, *OFFICIAL_REPO_SUBDIR.parts)
        if not os.path.isdir(extracted):
            # Fall back to standalone-layout ZIPs or any single extracted directory.
            if os.path.isdir(extracted_root):
                extracted = extracted_root
            for d in os.listdir(tmp_dir):
                candidate = os.path.join(tmp_dir, d)
                if os.path.isdir(candidate) and d != "__MACOSX":
                    subdir_candidate = os.path.join(candidate, *OFFICIAL_REPO_SUBDIR.parts)
                    extracted = subdir_candidate if os.path.isdir(subdir_candidate) else candidate
                    break
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("motis-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Motis behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Motis can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Motis couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Motis couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Motis behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `motis update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/DaddyElonMusk69/motis.git",
    "git@github.com:DaddyElonMusk69/motis.git",
    "https://github.com/DaddyElonMusk69/motis",
    "git@github.com:DaddyElonMusk69/motis",
}
OFFICIAL_REPO_URL = "https://github.com/DaddyElonMusk69/motis.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from motis_constants import get_motis_home
    return (get_motis_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from motis_constants import get_motis_home
        (get_motis_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Motis repository.")
        print("  This means you may miss updates from the official Motis repository.")
        print()
        try:
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "motis-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Motis to the latest version."""
    import shutil
    from motis_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Motis")
        return
    ...
    git_cwd = repo_root or PROJECT_ROOT
    git_dir = git_cwd / '.git'
    ...
    try:
        ...
        # the old bytecode (e.g. get_motis_home added to motis_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload motis_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_motis_home() added since the last release.
        try:
            import importlib
            import motis_constants as _hc
        ...
        try:
            from motis_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'motis config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: motis config migrate")
            else:
                print()
                print("Skipped. Run 'motis config migrate' later to configure.")
        else:
        ...
        try:
            ...
            # Discover all motis-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "motis-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. motis-gateway.service or motis-gateway-coder.service
            ...
            if is_macos():
                try:
                    from motis_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: motis gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: motis -p <profile> gateway run  for each profile)")
        ...
        print("  motis model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``motis -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from motis_cli.profiles import (
    ...
    if action is None:
        # Bare `motis profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_motis_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → motis -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  motis profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     motis -p {name} chat")
                else:
        ...
        name = args.profile_name
        from motis_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from motis_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from motis_cli.profiles import rename_profile
        try:
        ...
        from motis_cli.profiles import export_profile
        name = args.profile_name
        ...
        from motis_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from motis_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Motis log files."""
    from motis_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for motis CLI."""
    parser = argparse.ArgumentParser(
        prog="motis",
        description="Motis - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    motis                        Start interactive chat
    motis chat -q "Hello"        Single query mode
    motis -c                     Resume the most recent session
    motis -c "my project"        Resume a session by name (latest in lineage)
    motis --resume <session_id>  Resume a specific session by ID
    motis setup                  Run setup wizard
    motis logout                 Clear stored authentication
    motis auth add <provider>    Add a pooled credential
    motis auth list              List pooled credentials
    motis auth remove <p> <t>    Remove pooled credential by index, id, or label
    motis auth reset <provider>  Clear exhaustion status for a provider
    motis model                  Select default model
    motis config                 View configuration
    motis config edit            Edit config in $EDITOR
    motis config set model gpt-4 Set a config value
    motis gateway                Run messaging gateway
    motis -s github-auth,github-pr-workflow
    motis -w                     Start in isolated git worktree
    motis gateway install        Install gateway background service
    motis sessions list          List past sessions
    motis sessions browse        Interactive session picker
    motis sessions rename ID T   Rename/title a session
    motis logs                   View agent.log (last 50 lines)
    motis logs -f                Follow agent.log in real time
    motis logs errors            View errors.log
    motis logs --since 1h        Lines from the last hour
    motis update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Motis"
    )
    ...
    chat_parser.add_argument(
        "--provider",
        choices=["auto", "openrouter", "openai-codex", "copilot", "anthropic", "gemini", "huggingface", "zai", "kimi-coding", "minimax", "minimax-cn", "kilocode"],
        default=None,
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: motis-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Motis with an interactive wizard. "
                    "Run a specific section: motis setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for the Motis CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: motis-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Motis components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Motis setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Motis setup "
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Motis configuration"
    )
    ...
    def cmd_pairing(args):
        from motis_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from motis_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from motis_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. owner/repo-name)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from motis_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from motis_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'motis tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # motis tools disable <name...> [--platform cli]
    ...
    # motis tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from motis_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from motis_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'motis mcp add' to connect to a new server."
        ),
    )
    ...
    def cmd_mcp(args):
        from motis_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch motis --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            hermes_bin = shutil.which("motis")
            if hermes_bin:
                os.execvp(hermes_bin, ["motis", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "motis_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Motis"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Motis",
    ...
    def cmd_claw(args):
        from motis_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Motis to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Motis",
        description="Remove Motis from your system. Can keep configs/data for reinstall."
    )
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Motis instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Motis log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    motis logs                    Show last 50 lines of agent.log
    motis logs -f                 Follow agent.log in real time
    motis logs errors             Show last 50 lines of errors.log
    motis logs gateway -n 100     Show last 100 lines of gateway.log
    motis logs --level WARNING    Only show WARNING and above
    motis logs --session abc123   Filter by session ID
    motis logs --since 1h         Lines from the last hour
    motis logs --since 30m -f     Follow, starting from 30 min ago
    motis logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``motis -c Pokemon Agent Dev`` → ``motis -c 'Pokemon Agent Dev'``
