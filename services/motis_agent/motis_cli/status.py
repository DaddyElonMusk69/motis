"""
Status command for the Motis CLI.

Shows the status of all Motis components.
"""
...
from motis_cli.auth import AuthError, resolve_provider
from motis_cli.colors import Colors, color
from motis_cli.config import get_env_path, get_env_value, get_motis_home, load_config
from motis_cli.models import provider_label
from motis_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Motis components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                    ⚕ Motis Status                      │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from motis_cli.auth import get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (configure provider auth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (set a default provider)"
    ...
    if not terminal_env:
        ...
        # (motis status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if terminal_env == "ssh":
        ...
        modal_image = os.getenv("TERMINAL_MODAL_IMAGE", "nikolaik/python-nodejs:python3.11-nodejs20")
    ...
    if sys.platform.startswith('linux'):
        try:
            from motis_cli.gateway import get_service_name
            _gw_svc = get_service_name()
        except Exception:
            _gw_svc = "motis-gateway"
        try:
        ...
        from motis_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_motis_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    print(color("  Run diagnostics for detailed checks", Colors.DIM))
    print(color("  Open setup to configure providers", Colors.DIM))
    print()
