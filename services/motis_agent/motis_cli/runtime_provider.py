...
from motis_cli import auth as auth_mod
...
def _sanitize_requested_provider(value: Optional[str]) -> str:
    normalized = (value or "").strip().lower()
    if normalized == "nous":
        logger.info("Ignoring deprecated provider setting 'nous'; falling back to auto resolution.")
        return "auto"
    return normalized
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from motis_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if configured_mode and _provider_supports_explicit_api_mode(provider, configured_provider):
            ...
            from motis_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    requested = _sanitize_requested_provider(requested)
    if requested:
        return requested
    ...
    if isinstance(cfg_provider, str) and cfg_provider.strip():
        return _sanitize_requested_provider(cfg_provider)

    # Prefer the persisted config selection over any stale shell/.env
    # provider override so chat uses the endpoint the user last saved.
    env_provider = os.getenv("HERMES_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return _sanitize_requested_provider(env_provider)
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "motis-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
