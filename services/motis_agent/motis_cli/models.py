...
Add, remove, or reorder entries here — both `motis setup` and
`motis` provider-selection will pick up the change automatically.
"""
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with OpenRouter and similar OpenAI-compatible catalog endpoints.
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    _PROVIDER_ORDER = [
        "openrouter", "openai-codex", "copilot",
        "gemini", "huggingface",
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from motis_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Motis' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``motis_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from motis_cli.auth import resolve_api_key_provider_credentials
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from motis_cli.copilot_auth import copilot_request_headers
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from motis_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Motis will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
