...
- motis config edit     - Open config in editor
- motis config set      - Set a specific value
- motis config wizard   - Re-run setup wizard
"""
...
from motis_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_motis_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Motis is running in package-manager-managed mode.
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade motis-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "motis update"
...
def format_managed_message(action: str = "modify this Motis installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Motis installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.motis-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Motis installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade motis-agent"
        )

    return (
        f"Cannot {action}: this Motis installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Motis."
    )
...
def get_config_path() -> Path:
    ...
    return get_motis_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_motis_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the motis group can
...
def _ensure_motis_home_impl():
    ...
    if is_managed():
        ...
        try:
            _ensure_motis_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_motis_home_managed(home: Path):
...
def ensure_motis_home():
    """Backward-compatible alias for older Motis-named callers."""
    _ensure_motis_home_impl()
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Motis
        ...
        # Container resource limits (docker and modal — ignored for local/ssh)
        "container_cpu": 1,
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Motis sends a stable profile-scoped userId to Camofox
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Motis*" header.
...
# selection step, so this dict is intentionally empty — no single env var
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @motis:example.org)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'motis-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'motis setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Motis won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'motis doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: motis config set <key> <value>")
...
#   openai-codex (OAuth — motis auth) — OpenAI Codex
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Motis env var names, avoiding false positives from values
...
def show_config():
    ...
    print(color("│               ⚕ Motis Configuration                   │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  motis config edit     # Edit config file", Colors.DIM))
    print(color("  motis config set <key> <value>", Colors.DIM))
    print(color("  motis setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: motis config set <key> <value>")
            print()
            print("Examples:")
            print("  motis config set model anthropic/claude-sonnet-4")
            print("  motis config set terminal.backend docker")
            print("  motis config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'motis config migrate' to add them")
        ...
        print("  motis config           Show current configuration")
        print("  motis config edit      Open config in editor")
        print("  motis config set <key> <value>   Set a config value")
        print("  motis config check     Check for missing/outdated config")
        print("  motis config migrate   Update config with new options")
        print("  motis config path      Show config file path")
        print("  motis config env-path  Show .env file path")
        sys.exit(1)
