"""Slash command definitions and autocomplete for the Motis CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("update", "Update Motis to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    if not gated:
        return set()
    try:
        from motis_cli.config import read_raw_config
        cfg = read_raw_config()
    except Exception:
        return set()
    result: set[str] = set()
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``motis skills
