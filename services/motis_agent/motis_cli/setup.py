"""
Interactive setup wizard for Motis.
...
Config files are stored in ~/.motis/ for easy access.
"""
...
from motis_constants import get_optional_skills_dir
...
def _print_docs_link(path: str, *, label: str = "Guide") -> None:
    if not _DOCS_BASE:
        return
    print_info(f"   {label}: {_DOCS_BASE}/{path.lstrip('/')}")
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from motis_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from motis_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from motis_cli.config import get_env_value
    ...
    is_copilot_catalog_provider = provider_id == "copilot"
...
# display_motis_home imported lazily at call sites (stale-module safety during motis update)

from motis_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Motis Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Motis using environment variables or config commands:")
    print_info("  motis config set model.provider custom")
    print_info("  motis config set model.base_url http://localhost:8080/v1")
    print_info("  motis config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'motis setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'motis setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'motis setup' to configure"))
    ...
    web_backends = {
        "exa": bool(get_env_value("EXA_API_KEY")),
        "parallel": bool(get_env_value("PARALLEL_API_KEY")),
        "firecrawl": bool(get_env_value("FIRECRAWL_API_KEY") or get_env_value("FIRECRAWL_API_URL")),
        "tavily": bool(get_env_value("TAVILY_API_KEY")),
    }
    if web_backend and web_backends.get(web_backend, False):
        tool_status.append((f"Web Search & Extract ({web_backend})", True, None))
    elif any(web_backends.values()):
    ...
    browser_provider = str(browser_cfg.get("cloud_provider") or "local").strip().lower()
    browser_provider_label = {
        ...
        "camofox": "Camofox",
        "local": "Local browser",
    }.get(browser_provider, browser_provider or "Local browser")
    local_browser_ready = bool(
        shutil.which("agent-browser")
        or (PROJECT_ROOT / "node_modules" / ".bin" / "agent-browser").exists()
    )
    browser_available = False
    if browser_provider == "browserbase":
        browser_available = local_browser_ready and bool(
            get_env_value("BROWSERBASE_API_KEY") and get_env_value("BROWSERBASE_PROJECT_ID")
        )
    elif browser_provider == "browser-use":
        browser_available = local_browser_ready and bool(get_env_value("BROWSER_USE_API_KEY"))
        ...
        browser_available = bool(get_env_value("CAMOFOX_URL"))
    else:
        browser_available = local_browser_ready

    if browser_available:
        label = "Browser Automation"
        if browser_provider_label:
            label = f"Browser Automation ({browser_provider_label})"
        tool_status.append((label, True, None))
    else:
        missing_browser_hint = "npm install -g agent-browser, set CAMOFOX_URL, or configure Browser Use or Browserbase"
        if browser_provider_label == "Browserbase":
            missing_browser_hint = (
                "npm install -g agent-browser and set "
                "BROWSERBASE_API_KEY/BROWSERBASE_PROJECT_ID"
            )
        elif browser_provider_label == "Browser Use":
            missing_browser_hint = (
                "npm install -g agent-browser and set BROWSER_USE_API_KEY"
            )
        elif browser_provider_label == "Camofox":
            missing_browser_hint = "CAMOFOX_URL"
        elif browser_provider_label == "Local browser":
    ...
    if tts_provider == "elevenlabs" and get_env_value("ELEVENLABS_API_KEY"):
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'motis setup tts'"))
    else:
    ...
    if config.get("terminal", {}).get("backend") == "modal":
        if get_env_value("MODAL_TOKEN_ID") and get_env_value("MODAL_TOKEN_SECRET"):
            ...
            tool_status.append(("Modal Execution", False, "run 'motis setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'motis setup tools' to configure them,"
        )
        from motis_constants import display_motis_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {hermes_home}/state.db, cron/, logs/, sessions/ (gateway routes)"
    )
    print()
    ...
    print()
    print(f"   {color('motis setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('motis setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('motis setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('motis setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('motis setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('motis config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('motis config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('motis config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('motis', Colors.GREEN)}              Start chatting")
    print(f"   {color('motis gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('motis doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `motis tools` and `motis setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``motis model``)
    ...
    provider added to ``motis model`` is automatically available here.
    ...
    _print_docs_link("integrations/providers")
    print()

    # Delegate to the shared motis model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from motis_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: motis model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from motis_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Motis can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Motis can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'motis setup' or configure AUXILIARY_VISION_* settings")
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'motis setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Motis runs shell commands and code.")
    print_info("This affects tool execution, file access, and isolation.")
    _print_docs_link("developer-guide/environments")
    print()
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Motis via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("MOTIS_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `motis setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    _print_docs_link("user-guide/configuration")
    print()

    # ── Max Iterations ──
    current_max = get_env_value("MOTIS_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("MOTIS_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Motis delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    _print_docs_link("user-guide/messaging/slack", label="Full guide")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Motis delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Motis delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'motis whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'motis whatsapp' to choose your mode (separate bot number")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Motis to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    _print_docs_link("user-guide/messaging/webhooks", label="Full guide")
    print()
    ...
    print()
    print_info("   Open config in your editor:  motis config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Motis from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'motis setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     motis config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: motis gateway install")
            else:
                print_info("  You can install later: motis gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo motis gateway install --system")
                print_info("  Or run in foreground:  motis gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   motis gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `motis setup tools` and `motis tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from motis_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from motis_cli.auth import get_active_provider
...
# Config values may have different semantics between OpenClaw and Motis.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Motis to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Motis at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Motis at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Motis at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Motis at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Motis equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Motis config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Motis.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Motis's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Motis can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: motis claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: motis claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Motis configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Motis config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Motis (use motis claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      motis setup           — full or quick (auto-detected)
      motis setup model     — just model/provider
      motis setup terminal  — just terminal backend
      motis setup gateway   — just messaging platforms
      motis setup tools     — just tool configuration
      motis setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│      ⚕ Motis Setup — {label:<33s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│                ⚕ Motis Setup Wizard                    │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│   Let's configure your Motis installation.             │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Motis configured.")
        print()
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'motis setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'motis setup' again when ready.")
            return
        ...
        setup_mode = prompt_choice("How would you like to set up Motis?", [
    ...
    print()
    print_info("You can edit these files directly or use 'motis config edit'")
...
def _offer_launch_chat():
    ...
    print()
    if prompt_yes_no("Launch motis chat now?", True):
        from motis_cli.main import cmd_chat
        from types import SimpleNamespace
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'motis setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    motis setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  motis setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'motis setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Motis to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'motis setup gateway'.")
