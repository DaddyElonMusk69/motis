"""``motis logs`` — view and filter Motis log files.
...
Usage examples::
    ...
    motis logs                    # last 50 lines of agent.log
    motis logs -f                 # follow agent.log in real time
    motis logs errors             # last 50 lines of errors.log
    motis logs gateway -n 100     # last 100 lines of gateway.log
    motis logs --level WARNING    # only WARNING+ lines
    motis logs --session abc123   # filter by session ID substring
    motis logs --since 1h         # lines from the last hour
    motis logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_motis_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Motis runs — try 'motis chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_motis_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_motis_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_motis_home()}/logs/")
        return

    print(f"Log files in {display_motis_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'motis chat' to generate logs)")
