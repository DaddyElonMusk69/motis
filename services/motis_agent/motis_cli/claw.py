"""motis claw — OpenClaw migration commands.

Usage:
    motis claw migrate              # Interactive migration from ~/.openclaw
    motis claw migrate --dry-run    # Preview what would be migrated
    motis claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    motis claw cleanup              # Archive leftover OpenClaw directories
    motis claw cleanup --dry-run    # Preview what would be archived
"""
...
from motis_cli.config import get_motis_home, get_config_path, load_config, save_config
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: motis claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Motis")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'motis claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Motis migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: motis claw migrate --source /path/to/.openclaw")
        return
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: motis claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  motis claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  motis claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  motis config set OPENROUTER_API_KEY sk-or-v1-...")
