"""
Unified tool configuration for Motis.

`motis tools` and `motis setup tools` both enter this module.
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from motis_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "motis-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "motis-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "motis-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "motis-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "motis-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "motis-signal"},
    "bluebubbles": {"label": "💙 BlueBubbles", "default_toolset": "motis-bluebubbles"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "motis-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "motis-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "motis-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "motis-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "motis-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "motis-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "motis-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "motis-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "motis-webhook"},
}
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in the Motis project directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "motis-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "motis-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `motis tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by motis tools — default enabled
                enabled_toolsets.add(pts)
...
def _get_platform_runtime_toolsets(
    config: dict,
    platform: str,
    *,
    include_default_mcp_servers: bool = True,
) -> Set[str]:
    """Resolve the toolsets that should be exposed to a live agent session.

    ``_get_platform_tools`` is intentionally UI-oriented: it returns the
    checklist-style built-in toolsets that a user can toggle in ``motis tools``.
    That helper does not represent runtime-only core bundles like
    ``motis-finance`` or ``motis-operators`` because those are not exposed in
    the checklist. Standalone Motis sessions, however, still need those core
    toolsets.

    This helper starts from the UI-visible selection and then restores any
    non-configurable built-in toolsets that are part of the platform's default
    runtime bundle.
    """
    from toolsets import resolve_toolset
    ...
    default_tool_names = set(resolve_toolset(default_toolset))

    for runtime_toolset in ("motis-finance", "motis-operators", "messaging"):
        runtime_tool_names = set(resolve_toolset(runtime_toolset))
        if runtime_tool_names and runtime_tool_names.issubset(default_tool_names):
            enabled_toolsets.add(runtime_toolset)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (motis-cli, motis-telegram, etc.)
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str]) -> Set[str]:
    ...
    from motis_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `motis tools` and `motis setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Motis Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: see docs/ for tool configuration details", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_motis_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'motis' or gateway restart.", Colors.DIM))
    print()
