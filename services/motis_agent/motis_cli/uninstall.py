...
def remove_path_from_shell_configs():
    """Remove Motis PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing hermes-agent or motis PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Motis" comment and following line
                if '# Motis' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('motis' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing motis
                if 'motis' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the motis wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "motis",
        Path("/usr/local/bin/motis"),
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains motis_cli reference)
                content = wrapper.read_text()
                if 'motis_cli' in content or 'motis-agent' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def run_uninstall(args):
    ...
    print(color("│                ⚕ Motis Uninstaller                     │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Motis data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
    else:
        print("This will remove the Motis code but keep your configuration and data.")
    ...
    log_info("Removing motis command...")
    ...
    print()
    print("Thank you for using Motis! ⚕")
    print()
