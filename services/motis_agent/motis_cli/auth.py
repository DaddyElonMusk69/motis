"""
Multi-provider authentication system for Motis.

Supports external OAuth flows (OpenAI Codex, Qwen OAuth) and traditional
API key providers (OpenRouter, custom endpoints). Auth state is persisted
in ~/.hermes/auth.json with cross-process file locking.
...
from motis_cli.config import get_motis_home, get_config_path, read_raw_config
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from motis_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `motis model` to re-authenticate."

    if error.code == "subscription_required":
        return "This action requires an active subscription on the selected provider."

    if error.code == "insufficient_credits":
        return "The selected provider account is out of credits."
...
def _raise_unsupported_nous_error() -> None:
    raise AuthError(
        "Provider 'nous' is no longer supported by Motis. "
        "Choose another provider with 'motis model'.",
        provider="nous",
...
def _auth_file_path() -> Path:
    return get_motis_home() / "auth.json"
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    # Ignore legacy provider-specific "systems" payloads that no longer map to
    # supported Motis providers.
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        return {"version": AUTH_STORE_VERSION, "providers": {}, "active_provider": None}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `motis logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from motis_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'motis doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'motis model' for available providers, or run 'motis doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'motis model' to choose a "
...
#
# Motis maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Motis auth store (~/.hermes/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `motis auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `motis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `motis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `motis auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Motis auth store (~/.hermes/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Motis auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `motis auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Motis auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Motis's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Motis auth store")
            print("⚠️  Migrating Codex credentials to Motis's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `motis auth` to create a fully independent session.\n")
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Motis processes
...
# =============================================================================
# Deprecated provider compatibility shims
# =============================================================================
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Legacy status shim for stale config/auth-store references."""
    return {
        "logged_in": False,
        "configured": False,
        "provider": "nous",
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `motis auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `motis auth` and
    # `motis model` store device_code tokens.
    try:
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Motis can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Motis provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `motis auth` to create a separate session",
        })
...
def _prompt_model_selection(
    ...
    and unselectable. When *portal_url* is set, a follow-up message points
    the user to that provider page.
    """
    from motis_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        if _unavailable:
            ...
            if portal_url:
                ...
                print(f"{_DIM}  ── See {_upgrade_url} for locked models ──{_RESET}")
            else:
                print(f"{_DIM}  ── Some models are unavailable with the current account ──{_RESET}")
            print()
    ...
    if _unavailable:
        ...
        if portal_url:
            _upgrade_url = portal_url.rstrip("/")
            print(f"  {_DIM}── Unavailable models (see {_upgrade_url}) ──{_RESET}")
        else:
            print(f"  {_DIM}── Unavailable models for the current account ──{_RESET}")
        for mid in _unavailable:
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from motis_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'motis model' or 'motis setup' instead."""
    print("The 'motis login' command has been removed.")
    print("Use 'motis auth' to manage credentials,")
    print("'motis model' to select a provider, or 'motis setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    ...
    # Check for existing Motis-owned credentials
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            print("Existing Codex credentials found in Motis auth store.")
            try:
    ...
    if cli_tokens:
        ...
        print("Motis will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            ...
            print("Motis will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Motis gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Motis creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Motis auth store
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """Legacy shim for a removed provider."""
    _raise_unsupported_nous_error()
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Motis will use OpenRouter for inference.")
        else:
            print("Run `motis model` or configure an API key to use Motis.")
    else:
