...
def _get_bundled_skills_dirs() -> List[Path]:
    """Return bundled skill directories available from the source/package tree.

    This lets Hermes run directly from a checked-out repo without first copying
    skills into ``HERMES_HOME/skills``. The directories are treated the same way
    as read-only external skill dirs: local profile skills still take precedence.
    """
    ...
    for dirname in ("skills", "optional-skills"):
        candidate = (package_root / dirname).resolve()
        if not candidate.is_dir() or candidate == local_skills or candidate in seen:
            continue
        seen.add(candidate)
        bundled_dirs.append(candidate)

    return bundled_dirs


def get_external_skills_dirs() -> List[Path]:
    """Return validated external skill directories.
    ...
    2. Bundled ``skills/`` and ``optional-skills/`` directories next to the
       Hermes source tree/package, when present
    """
    config_path = get_hermes_home() / "config.yaml"
    local_skills = (get_hermes_home() / "skills").resolve()
    seen: Set[Path] = set()
    ...
    if config_path.exists():
        ...
        if isinstance(parsed, dict):
            ...
            if isinstance(skills_cfg, dict):
                config_dirs = skills_cfg.get("external_dirs")
                if isinstance(config_dirs, str):
                    config_dirs = [config_dirs]
                if isinstance(config_dirs, list):
                    raw_dirs.extend(str(entry).strip() for entry in config_dirs if str(entry).strip())
    ...
    for bundled_dir in _get_bundled_skills_dirs():
        if bundled_dir == local_skills or bundled_dir in seen:
            continue
        seen.add(bundled_dir)
