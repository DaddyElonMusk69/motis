...
_AUTH_JSON_PATH = get_motis_home() / "auth.json"
...
def _pool_runtime_api_key(entry: Any) -> str:
    if entry is None:
        return ""
    # Use the PooledCredential.runtime_api_key property when available.
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from motis_cli.auth import _read_codex_tokens
...
def _get_auxiliary_provider(task: str = "") -> str:
    ...
    then falls back to "auto".  Returns one of: "auto", "openrouter", "main".
    """
    if task:
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "codex":
        ...
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (configure provider auth)")
        return client, model
...
def _resolve_auto() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. If the user's main provider is NOT an aggregator,
      ...
      2. OpenRouter → custom → Codex → API-key providers.
    """
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (configure provider auth)")
                return None, None
    ...
    try:
        from motis_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("motis_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if task == "vision":
        ...
        if client is None:
            raise RuntimeError(
                ...
                f"Open setup to configure a provider."
            )
