"""Centralized logging setup for Motis.
...
def setup_logging(
    *,
    ...
    """Configure the Motis logging subsystem.
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_motis_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._motis_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from motis_cli.config import is_managed
...
def _add_rotating_handler(
    ...
    *,
    ...
    """Add a rotating handler to *logger*, skipping duplicates."""
