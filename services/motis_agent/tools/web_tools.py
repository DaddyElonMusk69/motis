...
def _build_data_mcp_headers(*, session_id: Optional[str] = None) -> Dict[str, str]:
...
def _normalize_search_envelope(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    data["web"] = [_normalize_search_result(result, idx) for idx, result in enumerate(raw_results)]
    data["total_results"] = data.get("total_results") if data.get("total_results") is not None else len(raw_results)
    normalized["data"] = data
    normalized["success"] = normalized.get("status") == "ok"
    return normalized
...
def _normalize_document_dict(document: Dict[str, Any]) -> Dict[str, Any]:
...
def _document_to_result(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "blocked_by_policy" in metadata:
        result["blocked_by_policy"] = metadata["blocked_by_policy"]
    return result
...
def _web_requires_env() -> list[str]:
    """Return tool metadata env vars for Motis Data MCP transport."""
    return [
        "DATA_MCP_URL",
        "AGENT_MCP_SECRET",
    ]
...
def _get_firecrawl_client():
    ...
    Requires direct Firecrawl credentials or a self-hosted Firecrawl URL.
    """
    ...
    if Firecrawl is None:
        raise ImportError(
            ...
            "Install it to use the Firecrawl web backend, or configure a different web backend."
        )
...
def web_search_tool(query: str, limit: int = 5, *, session_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.info("Searching the web through Motis Data MCP: '%s' (limit: %d)", query, limit)
        response_data = _normalize_search_envelope(
            _call_data_mcp_sync(
                "web_search",
                {
                    "query": query,
...
async def web_extract_tool(
    ...
    try:
        ...
        # ── Filter obviously unsafe or disallowed URLs before hitting the MCP ──
        safe_urls = []
        preblocked_documents: List[Dict[str, Any]] = []
        for url in urls:
            if not is_safe_url(url):
                preblocked_documents.append(
        ...
        documents: List[Dict[str, Any]] = list(preblocked_documents)
        if safe_urls:
            logger.info("Extracting content through Motis Data MCP: %d URL(s)", len(safe_urls))
            ...
            mcp_documents = response_data.get("documents")
            if not isinstance(mcp_documents, list):
                mcp_documents = []

            for raw_document in mcp_documents:
            ...
            response_payload = {
                ...
                "tool": "web_extract",
                "provider": "motis_agent_prefilter",
                "warnings": [],
                "error": None,
                "data": {"documents": []},
            }
        ...
        response = {
            "success": response_payload.get("status") == "ok",
            ...
            "data": {
                **dict(response_payload.get("data") or {}),
                "documents": documents,
            },
            "results": results,
        }
        ...
        if use_llm_processing and auxiliary_available:
            ...
            async def process_single_result(index: int, result: Dict[str, Any]):
                ...
                if processed:
                    ...
                    response["data"]["documents"][index]["content"] = processed
                    response["data"]["documents"][index].setdefault("metadata", {})
                    response["data"]["documents"][index]["metadata"]["summarized"] = True
            ...
            tasks = [process_single_result(idx, result) for idx, result in enumerate(results_list)]
        ...
        if trimmed_response.get("results") == [] and not trimmed_response.get("error"):
...
def check_firecrawl_api_key() -> bool:
    ...
    Availability is true when direct Firecrawl config is present.
...
if __name__ == "__main__":
    ...
    auxiliary_model_available = check_auxiliary_model()
...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Search the public web through the Motis Data MCP boundary. Returns relevant results with titles, URLs, descriptions, and provider metadata.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
