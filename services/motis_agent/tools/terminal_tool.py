...
A terminal tool that executes commands in local, Docker, Modal, and SSH environments.
Supports local execution, containerized backends, and direct Modal cloud sandboxes.

Environment Selection (via TERMINAL_ENV environment variable):
- "local": Execute directly on the host machine (default, fastest)
- "docker": Execute in Docker containers (isolated, requires Docker)
- "modal": Execute in Modal cloud sandboxes
- "ssh": Execute on a remote machine over SSH
...
# =============================================================================
# Persistent sandbox storage helpers
# =============================================================================
...
def _check_disk_usage_warning():
    ...
    try:
        storage_dir = get_sandbox_dir() / "docker"
        storage_dir.mkdir(parents=True, exist_ok=True)

        # Get total size of hermes directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(storage_dir / "*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    for failure in sudo_failures:
        if failure in output:
            from motis_constants import display_hermes_home as _dhh
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    Callers that cannot pipe subprocess stdin (modal) must embed the
...
# Allows runtime infrastructure to specify a custom Docker/Modal image for a
# specific task_id BEFORE work begins. When the terminal or file tools create a
# new sandbox for that task_id, they check this registry first and fall back to
# the TERMINAL_MODAL_IMAGE (etc.) env var if no override is set.
#
...
def register_task_env_overrides(task_id: str, overrides: Dict[str, Any]):
    ...
    Called by runtime infrastructure before work begins to configure per-task
    sandbox settings (e.g., a custom Dockerfile for the Modal image).
...
def _get_env_config() -> Dict[str, Any]:
    ...
    if env_type == "docker" and mount_docker_cwd:
        ...
        if (
            ...
            host_cwd = candidate
            cwd = "/workspace"
    elif env_type in ("modal", "docker") and cwd:
    ...
    return {
        ...
        # Container resource config (applies to docker and modal -- ignored for local/ssh)
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    Args:
        env_type: One of "local", "docker", "modal", "ssh"
        image: Docker/Modal image name (ignored for local/ssh)
    ...
    if env_type == "local":
        ...
        raise ValueError(f"Unknown environment type: {env_type}. Use 'local', 'docker', 'modal', or 'ssh'")
...
def is_persistent_env(task_id: str) -> bool:
    ...
    modal, etc.). Non-persistent backends still get torn down at end-of-turn
    to prevent leakage. The idle reaper
...
def cleanup_all_environments():
    ...
    # Also clean any orphaned persistent docker directories
    docker_root = get_sandbox_dir() / "docker"
    import glob
    for path in glob.glob(str(docker_root / "*")):
        try:
...
def terminal_tool(
    ...
    try:
        ...
        # Check per-task overrides set by runtime infrastructure before
        # falling back to global env var config.
...
def check_terminal_requirements() -> bool:
    ...
    try:
        if env_type == "local":
            ...
            logger.error(
                "Unknown TERMINAL_ENV '%s'. Use one of: local, docker, modal, ssh.",
                env_type,
            )
            return False
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_ENV: {os.getenv('TERMINAL_ENV', 'local')} (local/docker/modal/ssh)")
