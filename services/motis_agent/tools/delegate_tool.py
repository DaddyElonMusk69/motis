...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on another endpoint).
    """
    ...
    # Extract the parent's API key so subagents inherit auth.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or re-authenticate in the provider auth flow."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (motis_cli/config.py load_config()) so that
