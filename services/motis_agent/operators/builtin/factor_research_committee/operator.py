...
Auto-generated from vibe trading preset: factor_research_committee.yaml
...
class State(TypedDict):
    ...
    factor_type: str  # Factor type (value / momentum / quality / growth / alternative)
    task_mine_summary: str  # Output from factor_miner
    task_validate_summary: str  # Output from factor_validator
    task_combine_summary: str  # Output from factor_combiner
    task_review_summary: str  # Output from backtest_reviewer
...
MANIFEST = {
    "name": "Factor Research Committee",
    "version": 1,
    "type": "research",
...
6. **Live Deployment Recommendation** — Explicitly state "Recommend deployment / Deploy after improvement / Do not deploy"; list specific improvement directions and capacity upper bound recommendation""",
    ...
    "nodes": [
        {"name": "task_mine", "type": "REASON", "agent": "factor_miner"},
        {"name": "task_validate", "type": "REASON", "agent": "factor_validator"},
        {"name": "task_combine", "type": "REASON", "agent": "factor_combiner"},
        {"name": "task_review", "type": "REASON", "agent": "backtest_reviewer"},
    ],
...
async def task_mine(state: State) -> dict:
    """REASON: Factor Miner"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "factor_type": state.get("factor_type", ""),
    }
    context["upstream_context"] = ""
    result = await run_agent("factor_miner", context)
    log_event("task_mine", f"Completed: {len(result.summary)} chars")
    return {"task_mine_summary": result.summary}


async def task_validate(state: State) -> dict:
    """REASON: Factor Validator"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "factor_type": state.get("factor_type", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("factor_validator", context)
    log_event("task_validate", f"Completed: {len(result.summary)} chars")
    return {"task_validate_summary": result.summary}


async def task_combine(state: State) -> dict:
    """REASON: Factor Combiner"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "factor_type": state.get("factor_type", ""),
        # Upstream summaries
        "mine_result": state.get("task_mine_summary", ""),
        "validate_result": state.get("task_validate_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_mine_summary"):
        upstream_parts.append("## Mine Result\n" + state["task_mine_summary"])
    if state.get("task_validate_summary"):
        upstream_parts.append("## Validate Result\n" + state["task_validate_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("factor_combiner", context)
    log_event("task_combine", f"Completed: {len(result.summary)} chars")
    return {"task_combine_summary": result.summary}


async def task_review(state: State) -> dict:
    """REASON: Backtest Reviewer"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "factor_type": state.get("factor_type", ""),
        # Upstream summaries
        "combine_result": state.get("task_combine_summary", ""),
        "mine_result": state.get("task_mine_summary", ""),
        "validate_result": state.get("task_validate_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_combine_summary"):
        upstream_parts.append("## Combine Result\n" + state["task_combine_summary"])
    if state.get("task_mine_summary"):
        upstream_parts.append("## Mine Result\n" + state["task_mine_summary"])
    if state.get("task_validate_summary"):
        upstream_parts.append("## Validate Result\n" + state["task_validate_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("backtest_reviewer", context)
    log_event("task_review", f"Completed: {len(result.summary)} chars")
    return {"task_review_summary": result.summary}
...
def build_graph():
    g = StateGraph(State)
    g.add_node("task_mine", task_mine)
    g.add_node("task_validate", task_validate)
    g.add_node("task_combine", task_combine)
    g.add_node("task_review", task_review)

    g.set_entry_point("task_mine")
    g.add_edge("task_mine", "task_combine")
    g.add_edge("task_validate", "task_combine")
    g.add_edge("task_combine", "task_review")

    # Parallel entry: fan-out from first node to siblings
    g.add_edge("task_mine", "task_validate")
    g.add_edge("task_review", END)
