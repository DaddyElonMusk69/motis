...
Auto-generated from vibe trading preset: macro_strategy_forum.yaml
...
class State(TypedDict):
    market: str  # Focus market (e.g., A-shares, Hong Kong, global multi-asset, crypto)
    horizon: str  # Horizon (e.g., monthly, quarterly, annual)
    task_global_summary: str  # Output from global_economist
    task_domestic_summary: str  # Output from domestic_economist
    task_policy_summary: str  # Output from policy_analyst
...
MANIFEST = {
    ...
    "agents": {
        "global_economist": {
            ...
            "system_prompt": """You are a senior global economist at a sell-side research house, focused on Fed/ECB/BOJ policy, the global macro picture, and geopolitical risk. You provide top-tier international macro context for the macro strategy forum, emphasizing external channels that directly affect {market}.
...
Ground analysis in latest data; prefer read_url for timeliness.""",
        ...
        "domestic_economist": {
            ...
            "system_prompt": """You are a senior China economist at a sell-side house, focused on China macro, PBOC policy, fiscal policy, and the property cycle. You provide authoritative domestic analysis for the forum with a deep grasp of China’s policy logic.
...
Prefer fresh macro data via Tushare patterns; every judgment needs a data hook.""",
        ...
        "policy_analyst": {
            "role": "Policy Analyst",
            "system_prompt": """You are a senior policy analyst at a sell-side house, focused on regulation, industrial policy, tax and accounting changes, and their transmission to markets. You translate policy signals for the strategy team.
...
Use explicit numbers and dates; avoid vague “maybe”; attach confidence to each major call.""",
    ...
    "nodes": [
        {"name": "task_global", "type": "REASON", "agent": "global_economist"},
        {"name": "task_domestic", "type": "REASON", "agent": "domestic_economist"},
        {"name": "task_policy", "type": "REASON", "agent": "policy_analyst"},
...
async def task_global(state: State) -> dict:
    """REASON: Global Economist"""
    ...
    result = await run_agent("global_economist", context)
    log_event("task_global", f"Completed: {len(result.summary)} chars")
    return {"task_global_summary": result.summary}


async def task_domestic(state: State) -> dict:
    """REASON: China Economist"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "horizon": state.get("horizon", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("domestic_economist", context)
    log_event("task_domestic", f"Completed: {len(result.summary)} chars")
    return {"task_domestic_summary": result.summary}


async def task_policy(state: State) -> dict:
    ...
    context["upstream_context"] = ""
    result = await run_agent("policy_analyst", context)
    log_event("task_policy", f"Completed: {len(result.summary)} chars")
    return {"task_policy_summary": result.summary}
...
async def task_strategy(state: State) -> dict:
    ...
    context = {
        ...
        "horizon": state.get("horizon", ""),
        # Upstream summaries
        "global_macro": state.get("task_global_summary", ""),
        "domestic_macro": state.get("task_domestic_summary", ""),
        "policy_analysis": state.get("task_policy_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_global_summary"):
        upstream_parts.append("## Global Macro\n" + state["task_global_summary"])
    if state.get("task_domestic_summary"):
        upstream_parts.append("## Domestic Macro\n" + state["task_domestic_summary"])
    if state.get("task_policy_summary"):
        upstream_parts.append("## Policy Analysis\n" + state["task_policy_summary"])
...
def build_graph():
    g = StateGraph(State)
    g.add_node("task_global", task_global)
    g.add_node("task_domestic", task_domestic)
    g.add_node("task_policy", task_policy)
    g.add_node("task_strategy", task_strategy)

    g.set_entry_point("task_domestic")
    g.add_edge("task_global", "task_strategy")
    g.add_edge("task_domestic", "task_strategy")
    g.add_edge("task_policy", "task_strategy")

    # Parallel entry: fan-out from first node to siblings
    g.add_edge("task_domestic", "task_global")
    g.add_edge("task_domestic", "task_policy")
