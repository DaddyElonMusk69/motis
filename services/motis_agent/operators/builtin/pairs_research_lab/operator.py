...
Auto-generated from vibe trading preset: pairs_research_lab.yaml
...
class State(TypedDict):
    market: str  # Target market (e.g. A-shares, Hong Kong, US, crypto)
    sector: str  # Sector filter (e.g. banks, consumer, semis); empty = full market
    task_correlation_scan_summary: str  # Output from correlation_scanner
    task_cointegration_test_summary: str  # Output from cointegration_tester
    task_pair_strategy_summary: str  # Output from pair_strategist
    task_microstructure_review_summary: str  # Output from microstructure_reviewer
...
MANIFEST = {
    ...
    "agents": {
        "correlation_scanner": {
            ...
            "system_prompt": """You are a senior quant researcher focused on multi-asset correlation and pair candidate discovery, proficient in rolling correlation, comovement clustering, and within-/cross-sector screening across large universes.
...
Use factor_analysis for correlation and clustering.""",
        ...
        "cointegration_tester": {
            ...
            "system_prompt": """You are a senior econometrician focused on cointegration and mean-reversion modeling—Engle–Granger, Johansen trace, dynamic hedge ratio (OLS / Kalman), OU parameter estimation—with strict statistical validation of pair candidates.
...
Use factor_analysis for ADF, Johansen, Kalman where applicable.""",
        ...
        "pair_strategist": {
            ...
            "system_prompt": """You are a senior pair-trading strategist who turns statistical tests into fully specified, backtestable strategies—z-score entry, dynamic thresholds, stops, multi-pair portfolios—with institutional-grade documentation.
...
Use backtest with costs, dynamic hedge, multi-pair rebalancing.""",
        ...
        "microstructure_reviewer": {
            ...
            "system_prompt": """You are a senior microstructure analyst for pair execution—liquidity, spread modeling, impact, rebalancing cost—surfacing real-world friction and liquidity risk for stat-arb.
...
Use load_skill("market-microstructure"), load_skill("execution-model").""",
    ...
    "nodes": [
        {"name": "task_correlation_scan", "type": "REASON", "agent": "correlation_scanner"},
        {"name": "task_cointegration_test", "type": "REASON", "agent": "cointegration_tester"},
        {"name": "task_pair_strategy", "type": "REASON", "agent": "pair_strategist"},
        {"name": "task_microstructure_review", "type": "REASON", "agent": "microstructure_reviewer"},
    ],
...
async def task_correlation_scan(state: State) -> dict:
    """REASON: Correlation Scanner"""
    ...
    result = await run_agent("correlation_scanner", context)
    log_event("task_correlation_scan", f"Completed: {len(result.summary)} chars")
    return {"task_correlation_scan_summary": result.summary}


async def task_cointegration_test(state: State) -> dict:
    """REASON: Cointegration Tester"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "sector": state.get("sector", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("cointegration_tester", context)
    log_event("task_cointegration_test", f"Completed: {len(result.summary)} chars")
    return {"task_cointegration_test_summary": result.summary}


async def task_pair_strategy(state: State) -> dict:
    """REASON: Pair Strategist"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "sector": state.get("sector", ""),
        # Upstream summaries
        "correlation_report": state.get("task_correlation_scan_summary", ""),
        "cointegration_report": state.get("task_cointegration_test_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_correlation_scan_summary"):
        upstream_parts.append("## Correlation Report\n" + state["task_correlation_scan_summary"])
    if state.get("task_cointegration_test_summary"):
        upstream_parts.append("## Cointegration Report\n" + state["task_cointegration_test_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("pair_strategist", context)
    log_event("task_pair_strategy", f"Completed: {len(result.summary)} chars")
    return {"task_pair_strategy_summary": result.summary}


async def task_microstructure_review(state: State) -> dict:
    """REASON: Microstructure Reviewer"""
    # Build context from state
    context = {
        "market": state.get("market", ""),
        "sector": state.get("sector", ""),
        # Upstream summaries
        "strategy_report": state.get("task_pair_strategy_summary", ""),
        "correlation_report": state.get("task_correlation_scan_summary", ""),
        "cointegration_report": state.get("task_cointegration_test_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_pair_strategy_summary"):
        upstream_parts.append("## Strategy Report\n" + state["task_pair_strategy_summary"])
    if state.get("task_correlation_scan_summary"):
        upstream_parts.append("## Correlation Report\n" + state["task_correlation_scan_summary"])
    if state.get("task_cointegration_test_summary"):
        upstream_parts.append("## Cointegration Report\n" + state["task_cointegration_test_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("microstructure_reviewer", context)
    log_event("task_microstructure_review", f"Completed: {len(result.summary)} chars")
    return {"task_microstructure_review_summary": result.summary}
...
def build_graph():
    g = StateGraph(State)
    g.add_node("task_correlation_scan", task_correlation_scan)
    g.add_node("task_cointegration_test", task_cointegration_test)
    g.add_node("task_pair_strategy", task_pair_strategy)
    g.add_node("task_microstructure_review", task_microstructure_review)

    g.set_entry_point("task_cointegration_test")
    g.add_edge("task_correlation_scan", "task_pair_strategy")
    g.add_edge("task_cointegration_test", "task_pair_strategy")
    g.add_edge("task_pair_strategy", "task_microstructure_review")

    # Parallel entry: fan-out from first node to siblings
    g.add_edge("task_cointegration_test", "task_correlation_scan")
    g.add_edge("task_microstructure_review", END)
