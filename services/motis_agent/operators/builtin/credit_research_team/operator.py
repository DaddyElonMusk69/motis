...
Auto-generated from vibe trading preset: credit_research_team.yaml
...
class State(TypedDict):
    target: str  # Research subject or sector (e.g.: a specific LGFV platform, the property sector, steel bonds, AA-rated credit bond portfolio)
    market: str  # Bond market (default: China bond market; options: credit bonds / LGFV bonds / rate bonds / convertible bonds)
    task_credit_summary: str  # Output from credit_analyst
    task_rate_summary: str  # Output from rate_analyst
    task_sector_credit_summary: str  # Output from sector_credit_analyst
    task_strategy_summary: str  # Output from fixed_income_strategist
...
MANIFEST = {
    "name": "Fixed Income Credit Research Team",
    "version": 1,
    "type": "research",
...
Use the backtest tool to run historical backtests validating strategy performance across different rate / credit environments.""",
    ...
    "nodes": [
        {"name": "task_credit", "type": "REASON", "agent": "credit_analyst"},
        {"name": "task_rate", "type": "REASON", "agent": "rate_analyst"},
        {"name": "task_sector_credit", "type": "REASON", "agent": "sector_credit_analyst"},
        {"name": "task_strategy", "type": "REASON", "agent": "fixed_income_strategist"},
    ],
...
async def task_credit(state: State) -> dict:
    """REASON: Credit Analyst"""
    ...
    result = await run_agent("credit_analyst", context)
    log_event("task_credit", f"Completed: {len(result.summary)} chars")
    return {"task_credit_summary": result.summary}


async def task_rate(state: State) -> dict:
    """REASON: Interest Rate Analyst"""
    # Build context from state
    context = {
        "target": state.get("target", ""),
        "market": state.get("market", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("rate_analyst", context)
    log_event("task_rate", f"Completed: {len(result.summary)} chars")
    return {"task_rate_summary": result.summary}


async def task_sector_credit(state: State) -> dict:
    """REASON: Sector Credit Analyst"""
    # Build context from state
    context = {
        "target": state.get("target", ""),
        "market": state.get("market", ""),
    ...
    context["upstream_context"] = ""
    result = await run_agent("sector_credit_analyst", context)
    log_event("task_sector_credit", f"Completed: {len(result.summary)} chars")
    return {"task_sector_credit_summary": result.summary}


async def task_strategy(state: State) -> dict:
    """REASON: Fixed Income Strategist"""
    # Build context from state
    context = {
        "target": state.get("target", ""),
        "market": state.get("market", ""),
        # Upstream summaries
        "credit_report": state.get("task_credit_summary", ""),
        "rate_report": state.get("task_rate_summary", ""),
        "sector_credit_report": state.get("task_sector_credit_summary", ""),
    }
    # Build upstream context block
    upstream_parts = []
    if state.get("task_credit_summary"):
        upstream_parts.append("## Credit Report\n" + state["task_credit_summary"])
    if state.get("task_rate_summary"):
        upstream_parts.append("## Rate Report\n" + state["task_rate_summary"])
    if state.get("task_sector_credit_summary"):
        upstream_parts.append("## Sector Credit Report\n" + state["task_sector_credit_summary"])
    context["upstream_context"] = "\n\n".join(upstream_parts) if upstream_parts else "No upstream context available."
    result = await run_agent("fixed_income_strategist", context)
...
def build_graph():
    g = StateGraph(State)
    g.add_node("task_credit", task_credit)
    g.add_node("task_rate", task_rate)
    g.add_node("task_sector_credit", task_sector_credit)
    g.add_node("task_strategy", task_strategy)

    g.set_entry_point("task_credit")
    g.add_edge("task_credit", "task_strategy")
    g.add_edge("task_rate", "task_strategy")
    g.add_edge("task_sector_credit", "task_strategy")

    # Parallel entry: fan-out from first node to siblings
    g.add_edge("task_credit", "task_rate")
    g.add_edge("task_credit", "task_sector_credit")
