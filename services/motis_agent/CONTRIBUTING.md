# Contributing to Motis Agent

Thank you for contributing to Motis Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Motis to work everywhere.
...
Bundled skills (in `skills/`) ship with every Motis install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `motis skills browse` (labeled "official") and install it with `motis skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `motis skills install`.
...
├── cli.py                    # MotisCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (motis-cli, motis-telegram, etc.)
├── motis_state.py            # SQLite session database with FTS5 full-text search, session titles
...
│
├── motis_cli/               # CLI command implementations
...
- **Session persistence**: All conversations are stored in SQLite (`motis_state.py`) with full-text search and unique session titles. JSON logs go to `~/.hermes/sessions/`.
...
The user may skip setup and keep loading the skill. Motis only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `motis setup` or update `~/.hermes/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Motis tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `motis --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Motis uses a data-driven skin system — no code changes needed to add a new skin.
...
Add to `_BUILTIN_SKINS` dict in `motis_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `motis_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Motis runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Motis has terminal access. Security matters.
...
2. **Test manually**: Run `motis` and exercise the code path you changed
...
- Include: OS, Python version, Motis version (`motis version`), full error traceback
