...
class SessionEntry:
    ...
    # True once this session's legacy JSONL transcript has been migrated or
    # when the session was born directly into the SQLite-only Motis store.
    legacy_transcript_migrated: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "legacy_transcript_migrated": self.legacy_transcript_migrated,
        }
        if self.origin:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionEntry":
        ...
        return cls(
            ...
            legacy_transcript_migrated=data.get("legacy_transcript_migrated", False),
        )
...
class SessionStore:
    ...
    Uses SQLite (via SessionDB) as the source of truth for session metadata
    and message transcripts. sessions.json is retained only for routing the
    current session key -> session_id mapping. Legacy JSONL transcripts are
    read only for one-time migration into SQLite.
    """
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        try:
            ...
            print(f"[gateway] Warning: SQLite session store unavailable, transcript persistence disabled: {e}")
    ...
    def _db_session_kwargs(self, session_id: str) -> Dict[str, Optional[str]]:
        """Best-effort session metadata for ensuring a DB session row exists."""
        with self._lock:
            self._ensure_loaded_locked()
            for entry in self._entries.values():
                if entry.session_id != session_id:
                    continue
                return {
                    "source": entry.platform.value if entry.platform else "unknown",
                    "user_id": entry.origin.user_id if entry.origin else None,
                }
        return {"source": "unknown", "user_id": None}

    def _ensure_db_session_row(self, session_id: str) -> bool:
        """Ensure the SQLite conversation row exists for the session."""
        if not self._db:
            return False
        kwargs = self._db_session_kwargs(session_id)
        try:
            self._db.ensure_session(
                session_id=session_id,
                source=kwargs["source"] or "unknown",
                user_id=kwargs["user_id"],
            )
            return True
        except Exception as e:
            logger.debug("Could not ensure DB session row for %s: %s", session_id, e)
            return False

    def _is_legacy_transcript_migrated(self, session_id: str) -> bool:
        """Return whether this session should ignore legacy JSONL transcripts."""
        with self._lock:
            self._ensure_loaded_locked()
            return any(
                entry.session_id == session_id and entry.legacy_transcript_migrated
                for entry in self._entries.values()
            )

    def _mark_legacy_transcript_migrated(self, session_id: str) -> None:
        """Persist that SQLite is now authoritative for this session."""
        with self._lock:
            self._ensure_loaded_locked()
            changed = False
            for entry in self._entries.values():
                if entry.session_id != session_id or entry.legacy_transcript_migrated:
                    continue
                entry.legacy_transcript_migrated = True
                changed = True
            if changed:
                self._save()
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            entry = SessionEntry(
                ...
                legacy_transcript_migrated=True,
            )
    ...
    def switch_session(self, session_key: str, target_session_id: str) -> Optional[SessionEntry]:
        ...
        with self._lock:
            ...
            new_entry = SessionEntry(
                ...
                platform=old_entry.platform,
                chat_type=old_entry.chat_type,
                legacy_transcript_migrated=False,
            )
    ...
    def get_transcript_path(self, session_id: str) -> Path:
        """Get the path to a legacy JSONL transcript file for migration only."""
        return self.sessions_dir / f"{session_id}.jsonl"

    def _load_legacy_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        """Load a legacy JSONL transcript if one exists."""
        ...
        try:
            ...
            logger.debug("Failed to read legacy transcript %s: %s", session_id, e)
            return []

        return messages

    @staticmethod
    def _transcript_message_key(message: Dict[str, Any]) -> str:
        """Stable comparison key for transcript merge/dedup decisions."""
        normalized = {
    ...
    def _merge_legacy_and_db_transcript(
        self,
        legacy_messages: List[Dict[str, Any]],
        db_messages: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """Merge legacy JSONL history with any newer DB-only tail."""
        if not db_messages:
            return list(legacy_messages)

        legacy_keys = [self._transcript_message_key(msg) for msg in legacy_messages]
        db_keys = [self._transcript_message_key(msg) for msg in db_messages]

        overlap = 0
        max_overlap = min(len(legacy_keys), len(db_keys))
        for candidate in range(max_overlap, 0, -1):
            if legacy_keys[-candidate:] == db_keys[:candidate]:
                overlap = candidate
                break

        merged = list(legacy_messages)
        merged.extend(db_messages[overlap:])
        return merged
    
    def append_to_transcript(self, session_id: str, message: Dict[str, Any], skip_db: bool = False) -> None:
        """Append a message to the SQLite transcript store.

        Args:
            skip_db: When True, skip the write because the agent already
                     persisted directly to SQLite for this turn.
        ...
        if skip_db:
            return
        if not self._db:
            logger.debug("Skipping transcript append for %s because SQLite is unavailable", session_id)
            return

        self._ensure_db_session_row(session_id)
    ...
    def rewrite_transcript(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        ...
        SQLite is the sole durable transcript store.
        """
        if not self._db:
            logger.debug("Skipping transcript rewrite for %s because SQLite is unavailable", session_id)
            return
        ...
        try:
            ...
            self._mark_legacy_transcript_migrated(session_id)
        except Exception as e:
    ...
    def load_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        legacy_messages = []
        if not self._is_legacy_transcript_migrated(session_id):
            legacy_messages = self._load_legacy_transcript(session_id)

        # One-time migration path for older sessions. Merge the legacy JSONL
        # prefix with any newer DB-only tail, then mark SQLite authoritative.
        if self._db and legacy_messages:
            try:
                merged_messages = self._merge_legacy_and_db_transcript(
                    legacy_messages,
                    db_messages,
                )
                logger.info(
                    "Migrating legacy transcript %s into SQLite (%d legacy messages, %d existing DB messages, %d merged messages)",
                    session_id,
                    len(legacy_messages),
                    len(db_messages),
                    len(merged_messages),
                )
                if merged_messages != db_messages:
                    self.rewrite_transcript(session_id, merged_messages)
                    db_messages = self._db.get_messages_as_conversation(session_id)
                self._mark_legacy_transcript_migrated(session_id)
            except Exception as e:
                logger.debug("Legacy transcript migration failed for %s: %s", session_id, e)
                return merged_messages if "merged_messages" in locals() else legacy_messages
