...
SQLite conversation transcript so the receiving-side agent has context about
what was sent.
...
def _get_sessions_dir():
    """Return the active profile-scoped sessions directory."""
    return get_motis_home() / "sessions"


def _get_sessions_index():
    """Return the active session index path."""
...
def mirror_to_session(
    ...
    then writes a mirror entry to SQLite.
    ...
    try:
        ...
        _append_to_sqlite(session_id, mirror_msg, source=platform)
...
def _find_session_id(platform: str, chat_id: str, thread_id: Optional[str] = None) -> Optional[str]:
    ...
    """
    sessions_index = _get_sessions_index()
...
def _append_to_sqlite(session_id: str, message: dict, *, source: str) -> None:
