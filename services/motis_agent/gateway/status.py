...
The PID file lives at ``{MOTIS_HOME}/gateway.pid``. ``MOTIS_HOME`` defaults to
``~/.motis`` and ``HERMES_HOME`` remains a compatibility fallback. This means
separate Motis home directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "motis-gateway"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "gateway-locks"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting Motis home overrides."""
    home = get_motis_home()
...
def _get_lock_dir() -> Path:
    ...
    override = get_motis_env("GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "motis" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Motis gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different MOTIS_HOME dirs).
    """
