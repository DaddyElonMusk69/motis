...
# Resolve Motis home directory (prefers MOTIS_HOME, falls back to HERMES_HOME).
...
# Load environment variables from the active Motis home first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from motis_cli.env_loader import load_motis_dotenv
_env_path = _motis_home / '.env'
load_motis_dotenv(motis_home=_motis_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _setdefault_motis_env(name: str, value: str) -> None:
    """Set a Motis env var only when neither Motis nor Hermes is already set."""
    if get_motis_env(name) is None:
        set_motis_env(name, value)

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _motis_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                set_motis_env("MAX_ITERATIONS", str(_agent_cfg["max_turns"]))
            # Bridge agent.gateway_timeout → MOTIS_AGENT_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg:
                _setdefault_motis_env("AGENT_TIMEOUT", str(_agent_cfg["gateway_timeout"]))
            if "gateway_timeout_warning" in _agent_cfg:
                _setdefault_motis_env(
                    "AGENT_TIMEOUT_WARNING",
                    str(_agent_cfg["gateway_timeout_warning"]),
                )
        # Timezone: bridge config.yaml → MOTIS_TIMEZONE env var.
        # The user .env still takes precedence when already set.
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str):
            _setdefault_motis_env("TIMEZONE", _tz_cfg.strip())
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                set_motis_env("REDACT_SECRETS", str(_redact).lower())
    except Exception:
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _motis_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=get_motis_env("INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `motis skills config`"
                    )
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `motis skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse the active Motis config.yaml, returning {} on any error."""
    try:
        config_path = _motis_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _motis_home / 'config.yaml')
    return {}
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    """Resolve the Motis update command as argv parts.

    Tries in order:
    1. ``shutil.which("motis")`` — standard PATH lookup
    2. ``sys.executable -m motis_cli.main`` — fallback when Motis is running
       from a venv/module invocation and the ``motis`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("motis_cli") is not None:
            return [sys.executable, "-m", "motis_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    _VOICE_MODE_PATH = _motis_home / "gateway_voice_mode.json"
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks MOTIS_PREFILL_MESSAGES_FILE first, then the Hermes alias, then
        falls back to the prefill_messages_file key in the active config.yaml.
        Relative paths are resolved from the active Motis home.
        """
        import json as _json
        file_path = get_motis_env("PREFILL_MESSAGES_FILE", "") or ""
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _motis_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks MOTIS_EPHEMERAL_SYSTEM_PROMPT first, then the Hermes alias, then
        falls back to agent.system_prompt in the active config.yaml.
        """
        prompt = get_motis_env("EPHEMERAL_SYSTEM_PROMPT", "") or ""
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = get_motis_env("BACKGROUND_NOTIFICATIONS", "") or ""
        if not mode:
            try:
                import yaml as _y
                cfg_path = _motis_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Motis Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.motis/.env to allow open access, "
        ...
        if not notified and any(
            path.exists()
            for path in (
                _motis_home / ".update_pending.json",
                _motis_home / ".update_pending.claimed.json",
            )
        ):
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`motis pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _motis_home / ".update_response"
                try:
        ...
        _raw_stale_timeout = float(get_motis_env("AGENT_TIMEOUT", "1800") or "1800")
        ...
        if _quick_key in self._running_agents:
            ...
            from motis_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in motis_cli/commands.py — no hardcoded set to maintain here.
        from motis_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                ...
                # hyphens. See motis_cli/commands.py:_build_telegram_menu.
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_api_key = None
            try:
                _hyg_cfg_path = _motis_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Motis delivers cron job results "
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Motis venv) "
        ...
        try:
            ...
            # Save the full conversation to SQLite, including tool calls.
            ...
            # passed to the agent, not len(history) which may include metadata
            # entries from older imported transcripts.
            if not agent_failed_early:
                ...
                if not new_messages:
                    ...
                    # _flush_messages_to_session_db(), so skip the SessionStore
                    # DB write here to prevent the duplicate-write bug (#860).
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        profile_name = None
        for profiles_parent in (
            Path.home() / ".motis" / "profiles",
            Path.home() / ".hermes" / "profiles",
        ):
            try:
                rel = home.relative_to(profiles_parent)
                profile_name = rel.parts[0] if rel.parts else None
                break
            except ValueError:
                continue
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from motis_cli.commands import gateway_help_lines
        lines = [
            "📖 **Motis Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        import yaml
        from motis_cli.model_switch import (
        ...
        if persist_global:
            try:
                ...
                from motis_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `motis setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.motis/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Motis with the messaging extra, e.g. "
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _motis_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _run_background_task(
        ...
        try:
            ...
            enabled_toolsets = sorted(_get_platform_runtime_toolsets(user_config, platform_key))

            pr = self._provider_routing
            max_iterations = int(get_motis_env("MAX_ITERATIONS", "90") or "90")
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Motis to the latest version.

        Spawns ``motis update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``motis update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from motis_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `motis update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update Motis')}"
        ...
        if not hermes_cmd:
            return (
                "✗ Could not locate the `motis` command. "
                "Motis is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `motis update` manually in your terminal."
            )

        pending_path = _motis_home / ".update_pending.json"
        output_path = _motis_home / ".update_output.txt"
        exit_code_path = _motis_home / ".update_exit_code"
        ...
        # Spawn `motis update --gateway` detached so it survives gateway restart.
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Motis update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``motis update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _motis_home / ".update_pending.json"
        claimed_path = _motis_home / ".update_pending.claimed.json"
        output_path = _motis_home / ".update_output.txt"
        exit_code_path = _motis_home / ".update_exit_code"
        prompt_path = _motis_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Motis update finished.")
                    else:
                        await adapter.send(chat_id, "❌ Motis update failed (exit code {}).".format(exit_code))
                ...
                (_motis_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ Motis update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Motis update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Motis update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Motis update finished successfully."
                    else:
                        msg = "❌ Motis update failed. Check the gateway logs or run `motis update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        set_motis_env("SESSION_PLATFORM", context.source.platform.value)
        set_motis_env("SESSION_CHAT_ID", context.source.chat_id)
        if context.source.chat_name:
            set_motis_env("SESSION_CHAT_NAME", context.source.chat_name)
        if context.source.thread_id:
            set_motis_env("SESSION_THREAD_ID", str(context.source.thread_id))
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    ...
                    "users configure Motis features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                ...
                                "that can help users configure Motis features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _raw_tp
            or get_motis_env("TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            set_motis_env("SESSION_KEY", session_key or "")
            ...
            # Capture the tool definitions in the result metadata.
        ...
        try:
            ...
            # MOTIS_AGENT_TIMEOUT env var (Hermes alias still supported).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(get_motis_env("AGENT_TIMEOUT", "1800") or "1800")
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = float(
                get_motis_env("AGENT_TIMEOUT_WARNING", "900") or "900"
            )
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from motis_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    needing a separate `motis cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same MOTIS_HOME.
    # The PID file is scoped to MOTIS_HOME, so future multi-profile
    # setups (each profile using a distinct MOTIS_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            motis_home = str(get_motis_home())
            logger.error(
                "Another gateway instance is already running (PID %d, MOTIS_HOME=%s). "
                "Use 'motis gateway restart' to replace it, or 'motis gateway stop' first.",
                existing_pid, motis_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'motis gateway restart' to replace it,\n"
                f"   or 'motis gateway stop' to kill it first.\n"
                f"   Or use 'motis gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    log_dir = setup_logging(motis_home=_motis_home, mode="gateway")
    ...
    from motis_logging import _add_rotating_handler
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Motis Gateway - Multi-platform messaging")
