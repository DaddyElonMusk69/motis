    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_motis_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``motis sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the motis-api-server default.
        """
        ...
        enabled_toolsets = sorted(_get_platform_runtime_toolsets(user_config, "api_server"))
