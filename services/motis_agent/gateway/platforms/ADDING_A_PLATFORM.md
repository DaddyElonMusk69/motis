...
And add it to the `motis-gateway` composite:
...
## 12. Status Display (`motis_cli/status.py`)
...
## 13. Gateway Setup Wizard (`motis_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ motis_cli/ toolsets.py \
  --include="*.py" -l | sort -u
