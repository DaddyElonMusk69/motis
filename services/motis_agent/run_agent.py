...
from agent.motis_prompt import build_motis_runtime_prompt
...
from agent.runtime_context import reset_current_agent, set_current_agent
...
class AIAgent:
    ...
    def __init__(
        ...
        # Motis is a tool-using client by default.
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://motis.local",
                        "X-OpenRouter-Title": "Motis",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from motis_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        raise RuntimeError(
                            ...
                            f"variable, or choose a different provider in the model settings."
                        )
        ...
        try:
            from motis_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from motis_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from motis_constants import get_motis_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "motis"
                        except Exception:
                            pass
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        """
        import logging
        from motis_cli.providers import determine_api_mode
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``motis chat -q``. In that mode,
    ...
    _MEMORY_REVIEW_PROMPT = (
        ...
        "3. Has the conversation revealed a durable environment fact, stable workflow "
        "convention, or recurring tool quirk that will still matter in a later session?\n\n"
        "If something stands out, save it using the memory tool. "
        "Only save durable facts that would still be useful in a future session. "
        "Do NOT save greetings, task progress, summaries of what happened in this chat, "
        "'first interaction in this session', or notes that the user asked whether you "
        "remember them.\n\n"
    ...
    _COMBINED_REVIEW_PROMPT = (
        ...
        "Did the conversation reveal a durable environment fact, stable workflow convention, "
        "or recurring tool quirk? If so, save using the memory tool. "
        "Only save durable facts that will still matter later. Do NOT save greetings, "
        "task progress, summaries of this chat, 'first interaction in this session', "
        "or notes that the user asked whether you remember them.\n\n"
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        motis_prompt = build_motis_runtime_prompt(self.valid_tool_names)
        if motis_prompt:
            prompt_parts.append(motis_prompt)

        # Tool-aware behavioral guidance that remains useful on top of the
        # Motis core prompt.
        tool_guidance = []
        ...
        from motis_time import now as _hermes_now
        now = _hermes_now()
    ...
    def _interruptible_streaming_api_call(
            ...
            for chunk in stream:
                ...
                if delta and delta.content:
                    ...
                    if not tool_calls_acc:
                        _fire_first_delta()
                        partial_stream_text["text"] += delta.content
        ...
        if result["error"] is not None:
            if deltas_were_sent["yes"]:
                ...
                # Preserve the partial text and surface it as a truncated
                # response so the normal continuation logic can resume from the
                # exact prefix instead of silently accepting a cut-off answer.
                partial_text = partial_stream_text["text"]
                logger.warning(
                    "Partial stream delivered before error; returning synthetic "
                    "truncated response (%d chars) for recovery: %s",
                    len(partial_text),
                    result["error"],
                )
                if self.api_mode == "anthropic_messages":
                    ...
                    if partial_text:
                        partial_blocks.append(
                            SimpleNamespace(type="text", text=partial_text)
                        )
                    return SimpleNamespace(
                        id="partial-stream-stub",
                        model=getattr(self, "model", "unknown"),
                        content=partial_blocks,
                        stop_reason="max_tokens" if partial_text.strip() else "end_turn",
                        usage=None,
                    )
                ...
                return SimpleNamespace(
                    ...
                    choices=[SimpleNamespace(
                        index=0,
                        message=_stub_msg,
                        finish_reason="length" if partial_text.strip() else "stop",
                    )],
                    usage=None,
                )
            raise result["error"]
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self._is_qwen_portal():
            api_kwargs["metadata"] = {
                "sessionId": self.session_id or "motis",
        ...
        # specific. Only send them to OpenRouter-compatible endpoints.
        ...
        if self._supports_reasoning_extra_body():
            if _is_github_models:
                ...
                if self.reasoning_config is not None:
                    extra_body["reasoning"] = dict(self.reasoning_config)
                else:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from motis_cli.models import github_model_reasoning_efforts
    ...
    def flush_memories(self, messages: list = None, min_turns: int = None):
        ...
        flush_content = (
            ...
            "corrections, recurring patterns, and stable environment facts over task-specific details. "
            "Do not save session-meta notes like greetings, 'first interaction in this session', "
            "or the fact that the user asked whether you remember them.]"
        )
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        """
        _runtime_token = set_current_agent(self)
        try:
            ...
            reset_current_agent(_runtime_token)
    ...
    def run_conversation(
        ...
        # are reused.  The system prompt is Motis's territory; plugins
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            # cache prefix.  The system prompt is reserved for Motis internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Motis-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: motis config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: motis config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then re-authenticate from the provider auth flow.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Re-run provider setup.", force=True)
...
if __name__ == "__main__":
    try:
        ...
        print(
            "The optional 'fire' package is required to run run_agent.py directly. "
