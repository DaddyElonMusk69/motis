#!/bin/bash
# ============================================================================
# Motis Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and WSL2.
# Uses uv-managed Python environments for desktop and server installs.
#
# Usage:
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (Motis-managed)"
        HAS_NODE=true
        return 0
    fi
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Motis Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Motis behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up motis command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        HERMES_BIN="$(which motis 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "motis not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "motis entry point not found at $HERMES_BIN"
    ...
    # Create a user-facing shim for the motis command.
    mkdir -p "$command_link_dir"
    ln -sf "$HERMES_BIN" "$command_link_dir/hermes"
    log_success "Symlinked motis → $command_link_display_dir/hermes"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Motis Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so motis works immediately
    export PATH="$command_link_dir:$PATH"

    log_success "motis command ready"
}
...
Edit this to customize how Motis communicates with you.
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — Motis itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'motis setup' after install."
        return 0
    fi
    ...
    # Run motis setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m motis_cli.main setup < /dev/tty
    else
        python -m motis_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Motis to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'motis whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'motis whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'motis gateway install' later."
        return 0
    fi
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: motis gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: motis gateway"
            fi
        else
            ...
            log_info "To restart later: motis gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: motis gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}motis${NC}              Start chatting"
    echo -e "   ${GREEN}motis setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}motis config${NC}       View/edit configuration"
    echo -e "   ${GREEN}motis config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}motis gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}motis update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'motis' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
