# ============================================================================
# Motis Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$HermesHome = "$env:LOCALAPPDATA\motis",
    [string]$InstallDir = "$env:LOCALAPPDATA\motis\hermes-agent"
)
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Motis Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Motis-managed)"
        $script:HasNode = $true
        return $true
    }
...
function Set-PathVariable {
    Write-Info "Setting up motis command..."
    ...
    # Add the venv Scripts dir to user PATH so motis is globally available
    # On Windows, the motis.exe in venv\Scripts\ has the venv Python baked in
    ...
    # Only needed on Windows where we install to %LOCALAPPDATA%\motis instead
    ...
    Write-Success "motis command ready"
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m motis_cli.main setup
    } else {
        python -m motis_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\motis.exe"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'motis whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: motis gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: motis gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   motis              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   motis setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   motis config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   motis config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   motis gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   motis update       " -NoNewline -ForegroundColor Green
